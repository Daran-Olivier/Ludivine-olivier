...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadHeroImage(page, imgElementId) {
        try {
            const response = await fetch(`/backend-php/api/hero-images.php?page=${page}`);
            if (!response.ok) {
            ...
            const activeImage = data.images.find(img => img.is_active) || data.images[0];
            ...
            if (imgElement && activeImage) {
                ...
                testImg.onerror = function() {
                    console.warn(`Impossible de charger l'image API: ${activeImage.image_url}`);
                };
                testImg.src = activeImage.image_url;
            }

        } catch (error) {
            console.info(`API hero-images non disponible pour ${page}:`, error.message);
        }
    }
    ...
    window.changeHeroImage = async function(page, imageFile, altText, description = '') {
        if (!imageFile) {
        ...
        try {
            const response = await fetch('/backend-php/api/hero-images.php', {
                method: 'POST',
                body: formData
            });
