...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function addAPIProductsToExistingGrids() {
        try {
            ...
            if (!response.ok) {
                console.info('API backend pas encore déployée - contenu statique conservé');
                return;
            }

            const data = await response.json();
            if (!data.success || !data.products) {
                console.info('Aucun produit API disponible - contenu statique conservé');
                return;
            }
            ...
            Object.entries(categoryMapping).forEach(([sectionName, apiCategory]) => {
                ...
                const categoryProducts = products.filter(p => p.category === apiCategory);
                
                if (categoryProducts.length > 0) {
                    // Créer les cartes produit API avec le même style que les existantes
                    const apiCards = categoryProducts.map(product => createProductCard(product)).join('');
            ...
            console.info('API non disponible - contenu statique conservé:', error.message);
        }
    }

    // Fonction pour créer une carte produit compatible avec le style existant
    function createProductCard(product) {
        const img = product.image_url || '/backend-php/assets/placeholder.jpg';
        const price = product.price ? `${product.price}€` : '';
        const featured = product.is_featured ? '<span class="api-featured">🌟 API</span>' : '';
        
        return `
            <div class="product-card" data-product="api-${product.id}" data-source="api">
                <div class="product-image">
                    <img src="${img}" alt="${product.name}" onerror="this.src='/backend-php/assets/placeholder.jpg'">
                    <div class="product-overlay">
                        <button class="btn-modal" data-product="api-${product.id}">
                ...
                <div class="product-info">
                    ...
                    <div class="product-benefits">
                        ...
                        ${product.is_featured ? '<span class="benefit"><i class="fas fa-star"></i> En vedette</span>' : ''}
                    </div>
                </div>
            </div>
        `;
