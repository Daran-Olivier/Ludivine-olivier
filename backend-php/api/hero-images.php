...
try {
    ...
    switch ($method) {
        case 'GET':
            ...
            break;
        case 'POST':
            // Vérifier l'authentification pour la création
            if (!isLoggedIn()) {
                sendJSON(['success' => false, 'message' => 'Non authentifié'], 401);
            }
            handlePost($db);
            break;
        case 'PUT':
            // Vérifier l'authentification pour la modification
            if (!isLoggedIn()) {
                sendJSON(['success' => false, 'message' => 'Non authentifié'], 401);
            }
            handlePut($db);
            break;
        case 'DELETE':
            // Vérifier l'authentification pour la suppression
            if (!isLoggedIn()) {
                sendJSON(['success' => false, 'message' => 'Non authentifié'], 401);
            }
            handleDelete($db);
...
function uploadImage($file, $type = 'hero')
{
    ...
    if (move_uploaded_file($file['tmp_name'], $filepath)) {
        $url = '/backend-php/uploads/' . $type . '/' . $filename;
