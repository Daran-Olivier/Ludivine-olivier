...
const gameState = {
    ...
    downloadedPrograms: JSON.parse(localStorage.getItem('downloadedPrograms')) || []
};
...
function initScrollAnimations() {
    ...
    const animateElements = document.querySelectorAll('.feature-card, .product-card, .testimonial-card, .program-card');
...
function updateGameStats() {
    ...
    statNumbers.forEach(stat => {
        if (stat.dataset.target === '0' && stat.parentElement.querySelector('.stat-label').textContent.includes('Points')) {
            stat.textContent = gameState.points;
        } else if (stat.dataset.target === '0' && stat.parentElement.querySelector('.stat-label').textContent.includes('Badges')) {
            stat.textContent = gameState.badges.length;
        } else if (stat.dataset.target === '1' && stat.parentElement.querySelector('.stat-label').textContent.includes('Niveau')) {
...
function checkBadges() {
    const badges = [
        { id: 'explorer', name: 'Explorateur', condition: () => gameState.exploredProducts.length >= 5, points: 50 },
        { id: 'master', name: 'Maître de la Nutrition', condition: () => gameState.exploredProducts.length >= 15, points: 100 },
        { id: 'dedicated', name: 'Dévoué', condition: () => gameState.downloadedPrograms.length >= 2, points: 75 },
        { id: 'champion', name: 'Champion', condition: () => gameState.points >= 500, points: 150 }
    ];
...
function filterProducts(category) {
    ...
    const firstVisible = document.querySelector('.product-category[style="display: block"]') || 
...
function exploreProduct(productId, category) {
    ...
    if (!gameState.exploredProducts.includes(productKey)) {
        ...
        localStorage.setItem('exploredProducts', JSON.stringify(gameState.exploredProducts));
...
function updateCategoryProgress(category) {
    const categoryProducts = gameState.exploredProducts.filter(p => p.startsWith(category));
    const categorySection = document.getElementById(category);
    
    if (categorySection) {
        const progressBar = categorySection.querySelector('.progress-fill-category');
...
function initProgramQuestionnaire() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const activites = Array.from(form.querySelectorAll('input[name="activite"]:checked')).map(a => a.value);
...
const programmes = [
    ...
    },
    {
        ...
        description: 'Pour ceux qui veulent prendre du muscle avec des exercices avancés.',
        objectif: 'muscle',
        niveau: 'avance',
        activites: ['muscu'],
...
function afficherProgrammes(objectif, niveau, activites) {
    ...
    const filtres = programmes.filter(p =>
        p.objectif === objectif &&
        p.niveau === niveau &&
        p.activites.some(a => activites.includes(a))
    );
    if (filtres.length === 0) {
        zone.innerHTML = '<p>Aucun programme ne correspond à tes réponses. Essaie d’autres options !</p>';
        return;
    }
    filtres.forEach(p => {
        const card = document.createElement('div');
        card.className = 'program-card';
        card.style = 'background:#fff; border-radius:8px; box-shadow:0 2px 8px #ccc; margin-bottom:1em; padding:1em;';
...
function initQuiz() {
    ...
    quizOptions.forEach(option => {
        option.addEventListener('click', function() {
            const question = parseInt(this.closest('.quiz-question').dataset.question);
...
function nextQuizStep() {
    ...
    document.querySelector(`.quiz-question[data-question="${currentQuizStep}"]`).classList.add('active');
}
...
function showQuizResult() {
    ...
    const programCard = document.querySelector(`.program-card[data-program="${recommendedProgram}"]`);
    if (programCard) {
...
function downloadProgram(programId) {
    if (!gameState.downloadedPrograms.includes(programId)) {
        ...
        localStorage.setItem('downloadedPrograms', JSON.stringify(gameState.downloadedPrograms));
...
function generateCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        calendarHTML += `
            <div class="${classes}" onclick="selectDate('${date.toISOString()}')" ${isPast ? 'style="pointer-events:none;opacity:0.3;"' : ''}>
                ${day}
            </div>
        `;
    }
...
function generateTimeSlots(date) {
    ...
    slots.forEach(time => {
        ...
        slotsHTML += `
            <button class="time-slot ${isAvailable ? 'available' : 'unavailable'}" 
                    ${isAvailable ? `onclick="selectTime('${time}')"` : 'disabled'}>
                ${time}
                ${!isAvailable ? '<span class="unavailable-label">Complet</span>' : ''}
            </button>
        `;
    });
...
function selectConsultationType(type) {
    ...
    document.querySelectorAll('.consultation-card').forEach(c => c.classList.remove('selected'));
...
function updateBookingStep() {
    ...
    document.getElementById('prevBtn').style.display = bookingStep > 1 ? 'inline-flex' : 'none';
    document.getElementById('nextBtn').style.display = bookingStep < 4 ? 'inline-flex' : 'none';
...
function handlePartnershipSubmit(e) {
    ...
    // Afficher un message de succès
    showSuccessMessage('Votre candidature a été envoyée avec succès ! Nous vous contacterons sous 48h.');
...
console.log('🎮 Système de gamification activé !');
console.log('💪 Points actuels:', gameState.points);
console.log('🏆 Badges débloqués:', gameState.badges.length);
console.log('📈 Niveau:', gameState.level);